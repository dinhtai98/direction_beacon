...
import 'package:direction_device/utils.dart';
...
abstract class Beacon {
  ...
  double get distance {
    ...
    // double rssiCorrection = 0.96 + pow((rssi).abs(), 3.0) % 10.0 / 150.0;
    // if (ratio < 1.0) {
    //   return pow(ratio, 9.98) * rssiCorrection;
    // } else {
    //   return ((0.103 + 0.89978) * pow(ratio, 7.71)) * rssiCorrection;
    // }
    double ratio = rssi * 1.0 / (txAt1Meter);
    if (ratio < 1.0) {
      return pow(ratio, 10);
  ...
  static List<Beacon> fromScanResult(ScanResult scanResult) {
    return <Beacon>[
      ...
      EddystoneURL.fromScanResult(scanResult),
...
class EddystoneURL extends Eddystone {
  final String url;
  ...
  factory EddystoneURL.fromScanResult(ScanResult scanResult) {
    if (!scanResult.advertisementData.serviceData
        .containsKey(EddystoneServiceId)) {
      return null;
    }
    ...
    var frameType = rawBytes[0];
    var tx = byteToInt8(rawBytes[1]);
    var url = uRLPrefixFromByte(rawBytes[2]);
    List<int> tt = [];
    for (var i = 3; i < rawBytes.length - 1; i++) {
      tt.add(rawBytes[i]);
    }
    // String t = utf8.decode(tt);
    // url = url + t;
    // url = url + encodedStringFromByte(rawBytes[rawBytes.length - 1]);
    return EddystoneURL(
        frameType: frameType,
        tx: tx,
        url: url,
  ...
  int get hash => hashObjects(
      ["EddystoneURL", EddystoneServiceId, this.frameType, this.tx, this.url]);
}
